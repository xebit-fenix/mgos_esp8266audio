...
AudioOutputMixerStub::AudioOutputMixerStub(AudioOutputMixer *sink, int id) : AudioOutput()
{
  ...
  this->parent = sink;
  SetGain(1.0);
  this->rate_compensator = 0;
  this->source_rate = sink->GetRate();
  this->destination_rate = sink->GetRate();
}
...
{
  audioLogger->printf("Set Mixer stub %d to %d hz", this->id, hz);
  this->source_rate = hz;
  this->destination_rate = this->parent->GetRate();
  return true;
...
bool AudioOutputMixerStub::ConsumeSample(int16_t sample[2])
{
  ...
  while (this->rate_compensator < this->destination_rate) {
    bool ret = parent->ConsumeSample(amp, id);
    if (ret == false) return false;
    this->rate_compensator += this->source_rate;
  }
  this->rate_compensator -= this->destination_rate;
  return true;
}
